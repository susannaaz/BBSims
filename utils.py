...
def get_beta_map(nside, beta0, amp, gamma, l0=80, l_cutoff=2, seed=None, gaussian=True, mean_pars=None):
    """
    ...
    if gaussian:
        ...
        if mean_pars['dust_beta_map']=='GNILC': #d10
            beta_dust = hp.ud_grade(hp.read_map('./data/gnilc_dust_beta_nside256_equatorial.fits', verbose=False), nside_out=nside)
        else: #dm/d1
...
def get_beams(nside, mean_pars):
    """ Generate the beams in Fourier space.
        Inputs:
        nside: HEALPix resolution parameter.
        unit_beams: decide whether to create unitary or SO-like beams. 
    Returns:
        A dictionary containing ells and beam values for each frequency in band_names.
    """
    ls = np.arange(3*nside)
    stout = ''
    freqs = get_freqs()
...
def get_theory_sacc(nside, mean_pars=None, moment_pars=None, delta_ell=10, add_11=False, add_02=False):
    ...
    """
    import sys
    sys.path.append('/mnt/zfsusers/susanna/BBPower/sacc/sacc')
    import sacc
    ...
    # Bins/bandpowers
    larr_all = th['ls_unbinned']
    l_eff = th['ls_binned']
    windows = th['windows'] #[N_bins, nls] or [nbands, nls]
    nbands, nls = windows.shape
    s_wins = sacc.BandpowerWindow(larr_all, windows.T)
    
    # Binned Cls
    #bpw_freq_sig = th['cls_binned'] #[n_pairs, nell]
    cl_unbinned = th['cls_unbinned'] #(nfreqs, npol, nfreqs, npol, nls)
    bpw_freq_sig = np.dot(cl_unbinned, windows.T) #(nfreqs, npol, nfreqs, npol, nbands)
    bpw_freq_sig = bpw_freq_sig.reshape([nfreq*npol, nfreq*npol, nbands]) #equivalent in save_cl
    ...
    # TODO: add non-unit beams/bpss at some point
    ##sys.path.append('/mnt/zfsusers/susanna/BBHybrid/')
    ##import noise_calc2 as nc
    ##import utils2 as u
    ##band_names = ['LF1', 'LF2', 'MF1', 'MF2', 'UHF1', 'UHF2']
    ### Bandpasses
    ##bpss = {n: u.Bpass(n,f'/mnt/zfsusers/susanna/BBHybrid/data/bandpasses/{n}.txt') for n in band_names}
    ...
    for inu, n in enumerate(nus):
        map_names.append('band%d' % (inu+1) + '_E')
        map_names.append('band%d' % (inu+1) + '_B')
    for ii, (i1, i2) in enumerate(zip(indices_tr[0], indices_tr[1])):
        ...
        cl_type = f'cl_{p1}{p2}'
        s.add_ell_cl(cl_type, n1, n2, l_eff, bpw_freq_sig[i1, i2, :], window=s_wins)

    #s.save_fits(f'{prefix_out}cls_fid_{sname}_{skn}.fits', overwrite=True)
    #s.save_fits(f'prova_cls_fid.fits', overwrite=True)

    #wait for now
    #cov_bpw = th['cov_binned'] #[n_pairs, n_ell, n_pair, n_ell]
...
def get_sky_realization(nside, plaw_amps=True, gaussian_betas=True, seed=None,
    ...
    if gaussian_betas:
        ...
        beta_sync, beta_dust = get_beta_map(nside, None, None, None, None, None,
                                            gaussian=False, mean_pars=mean_pars)
    ...
    if mean_pars['dust_SED'] == 'mbb':
        ...
        d2 = models("d5", nside)
    #elif mean_pars['dust_SED'] == 'curved_plaw': #TODO
    #    d2 = models("d4", nside)
    else:
    ...
    d2[0]['A_U'] = U_dust
    # Sync
    if mean_pars['sync_SED'] == 'plaw':
        s1 = models("s1", nside)
    #elif mean_pars['sync_SED'] == 'curved_plaw': #TODO
    #    s1 = models("s3", nside)
    ...
    instrument_config = {
        ...
        'add_noise' : False,
        'use_bandpass' : False,
        'channel_names' : band_names, #['LF1', 'LF2', 'MF1', 'MF2', 'UHF1', 'UHF2'],
        'output_units' : 'uK_RJ',
